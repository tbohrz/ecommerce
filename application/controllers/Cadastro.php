...
class Cadastro extends CI_Controller {
    ...
    public function enviar_email_confirmacao($dados){               
        ...
        $this->email->from("loja@GMT","E-commerce GMT Eletrônicos");
        $this->email->to($dados['email']);
        $this->email->subject('E-commerce GMT Eletrônicos - Confirmação de cadastro');
    ...
    public function recuperar_login(){
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            if(count($cliente)==1){
                ...
                $this->email->from("loja@GMT","E-commerce GMT Eletrônicos");
                $this->email->to($dados->email);
                $this->email->subject('E-commerce GMT Eletrônicos - Recuperação de cadastro');
                $this->email->message($mensagem);            
                if($this->email->send()){
                    $data_header['categorias'] = $this->categorias;        
                    $this->load->view('html-header');
